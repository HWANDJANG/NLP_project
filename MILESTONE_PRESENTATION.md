...
## 9. Next Steps — 방향 전환: Fine-tuning → Retrieval-Augmented Generation

### 9-0. 왜 방향을 바꾸는가 (지금까지의 실험이 내린 결론)

여기까지의 실험은 하나의 일관된 메시지로 수렴한다 — **"우리가 가진 2,393개 데이터를 LM에 직접 학습시키는 접근은 이 규모·불균형에서 한계가 크다."**

| 단계 | 시도 | 결과 |
|---|---|---|
| 분류 (§7-1~7-3) | 2,393개로 분류기 학습 | minority recall 50%·majority baseline 못 넘음 — **데이터가 천장** |
| 증강 (§7-4) | Gemini로 train s/c 늘려 재학습 | macro-F1 0.599→0.554 **하락** — 합성 데이터로도 학습 한계 극복 못 함 |

분류조차 이런데, **생성(generation) fine-tuning은 더 불리하다**:
- conditional generation의 학습 단위는 `(board, title, post) → comment`인데 **unique 입력은 게시글 410개**뿐
- board×style 조합은 더 희박 (정치+supportive = 단 1개) → fine-tuning으로는 절대 학습 불가

#### "그럼 데이터를 더 크롤링하고 라벨링하면 되지 않나?" — 고려했으나 fine-tuning의 해법이 못 됨

추가 수집은 처음 계획(발표 후 추가 크롤링+직접 라벨링)에도 있었으나, 다음 이유로 **"더 모으기"는 비용 대비 효과가 낮다**고 판단했다:

1. **라벨링 비용이 비현실적** — minority 를 의미 있게 늘리려면(s 82→~300) s 발생률이 5~8%뿐이라 **3,000~4,000개 댓글을 단일 annotator(본인)가 전수 검토**해야 함. 학부 프로젝트 기간·인력으로는 무리.
2. **늘려도 불균형은 그대로** — 자연 분포라 추가 크롤링하면 i 도 같이 쏟아짐(85%). 라벨링 수십 시간을 써도 i:c:s 비율 자체는 거의 안 변함.
3. **규모의 차원이 다름** — 분류는 클래스당 수백이면 어느 정도 되지만, **generation fine-tuning 은 수만 입력-출력 쌍** 수준을 요구. 게시글 410개를 1,000개로 늘려도 임계점에 한참 못 미침.
4. **증강 실험이 이미 답을 줌** — '양 늘리기'(합성)는 §7-4 에서 오히려 역효과로 확인됨. 자연 데이터로 양을 조금 늘려도 *fine-tuning 패러다임의 한계*는 동일하다는 신호.

> **결론: 문제는 "데이터가 부족하다"가 아니라 "이 데이터를 *학습*으로 쓰는 방식이 안 맞는다"이다.** 더 모으는 데 시간을 쓰기보다, 있는 데이터를 *다르게* 쓰는 편이 ROI 가 높다. RAG 는 데이터 '양'이 아니라 '검색 품질'에 의존하므로 기존 410글·2,393댓글로 즉시 작동한다.

> **따라서 기말 단계에서는 수집한 라벨 데이터를 직접 학습시키기보다, 게시판·스타일별 예시를 검색해 few-shot prompt 로 넣는 Retrieval-Augmented Generation(RAG) 방식으로 전환한다.**
> 데이터를 "학습 대상"이 아니라 **"검색 예시 풀(example pool)"** 로 재정의하면, 410개·2,393개는 충분히 쓸 만한 자원이 된다. 정치+supportive 가 1개여도, 그 1개를 예시로 넣으면 LLM 이 톤을 모방할 수 있다 — fine-tuning 으로는 불가능했던 것이 RAG 로는 가능.

### 9-1. Style example pool 재구성
- 기존 라벨 데이터셋을 `board, title, post, comment, label` 기준으로 정리
- d/delete 제외, 너무 짧거나 맥락 없는 댓글은 example 에서 제외

### 9-2. Retrieval module
- 입력: `[BOARD] [STYLE] [TITLE] [POST]`
- 1차 필터: 같은 board + 같은 style
- 2차 검색: title/post 와 의미적으로 유사한 기존 게시글 검색
- 초기 TF-IDF 기반 → 가능하면 sentence embedding(예: ko-sroberta) 기반으로 확장

### 9-3. Few-shot prompt 구성
- 검색된 실제 댓글 3~5개를 예시로 삽입
- 게시판 문화·스타일 조건 명시, 출력은 실제 커뮤니티 댓글처럼 짧고 자연스럽게 제한

### 9-4. LLM 기반 댓글 생성 (Gemini)
- 같은 글에 board/style 조건을 바꿔 생성 비교 (예: 취업+supportive vs 정치+critical)
- → PROJECT_PLAN 의 "조건 변경 효과" 실험을 RAG 형태로 계승

### 9-5. 평가
- **핵심 ablation — zero-shot vs retrieval-few-shot**: 예시 없이 생성 vs 검색 예시를 넣어 생성 →
  style accuracy 차이를 분류기로 정량 비교 → **"retrieval 이 style 제어에 실제로 기여한다"** 를 입증
  (단순 LLM 호출과의 차별점 = 본 프로젝트의 학술적 기여)
- 자동: 기존 style classifier 를 **보조 평가기**로 사용
  - ⚠️ 분류기 macro-F1 ≈ 0.6 수준 → 성능 보장 도구가 아니라 **style consistency 확인용**으로만 사용
- 정성: 사람이 자연스러움 / 스타일 일치도 / 게시판 적합성 평가

### 9-6. 최종 데모 — Community Reaction Simulator
- 사용자가 글 제목·본문 입력 → 게시판·스타일 조건에 따라 서로 다른 댓글 생성
- 같은 글에 s/i/c 3가지 반응을 동시에 보여주는 형태

### (선택) example pool 보강
- RAG 품질은 example pool 다양성에 비례 → 발표 후 정치(c)·취업/연애(s) 추가 크롤링은 **학습용이 아니라 검색 풀 확충용**으로 여전히 유효
...
│   ├── augmentation/             # Gemini paraphrase 증강 (train-only) + split 고정
│   ├── eda/                      # 차트·통계
│   ├── classifier/               # baseline, baseline_2class, klue_bert
│   ├── retrieval/                # (기말 예정) board/style/topic 기반 example 검색
│   └── generation/               # (기말 예정) few-shot prompt 구성 + Gemini 생성
│
├── data/
...
> baseline 실패 진단(majority < 모델 정확도) → 2-class 재구성 → KLUE-BERT(minority recall 2배) →
> LLM 증강 역효과(negative result)까지 시행착오를 거치며 '2,393개 데이터를 LM에 직접 학습시키는 접근의 한계'를 정량적으로 확인했다.
> 이 결론에 근거해, 기말 단계에서는 fine-tuning 대신 데이터를 검색 예시 풀로 쓰는
> Retrieval-Augmented Generation(검색 + few-shot)으로 방향을 전환한다."**
...
11. 데이터 증강 전략 + 증강 전/후 비교 (Gemini, train-only / 원본 test 평가) → negative result
12. **방향 전환: fine-tuning → RAG** — "데이터를 학습이 아니라 검색 예시로" + RAG 파이프라인(검색→few-shot→생성) + ablation 계획
13. 한 줄 요약 (위 메시지) + 최종 데모(Community Reaction Simulator) 미리보기
